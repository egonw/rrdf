...
After RDF and SPARQL integration was finished, I realized that impact would even be larger if
I would add support for that to R. That was in 2011. This package does just that, and allows
to work with RDF data and perform SPARQL queries. I am indepted to Rajarshi Guha, whos rcdk
package provided me with a template to create this package from.

This tutorial will demo in two sections how RDF can be handles and how SPARQL functionality
can be used.
...
The store is in fact a Jave object form the Jena library, so that simply printing the object
...
We can see now that we have one more triple than we had earlier. The other method is
for adding data triples, and now the last parameter is a Literal:
...
<<>>=
save.rdf(store, "/tmp/store.rdfxml")
@
...
<<>>=
save.rdf(store, "/tmp/store.ardfxml", format="RDF/XML-ABBREV")
save.rdf(store, "/tmp/store.n3", format="N3")
@
...
save.rdf(store, "/tmp/prefix.n3", format="N3")
@
